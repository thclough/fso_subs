...
blogsRouter.post('/', middleware.userExtractor, async (request, response) => {
  ...
  const savedBlogJoin = await Blog.findById(savedBlog._id).populate('user', { username: 1, name: 1, id: 1 })
...
blogsRouter.put('/:id', middleware.userExtractor, async (request, response) => {
  ...
  if (user.id.toString() === targetBlog.user.toString() || request.body.likes - targetBlog.likes === 1) {
  ...
  response.json(populatedBlog)
    ...
    response.status(401).json({ error: 'you are not the creator of this blog and cannot edit this field' })
  }
})

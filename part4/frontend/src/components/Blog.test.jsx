...
describe('<Blog />', () => {
  ...
  beforeEach(() => {
    ...
    container = render(
        <Blog key={testBlog.id} blog={testBlog} onLikeClick={decFuncLike} onDelete={decFuncDel} userData={testUser}/>
    ).container
  })
  ...
  test('renders title and author by default, BUT hides url and number of likes is not visible', async () => {
    ...
    const element2 = screen.queryByText(`url: ${testBlog.url} likes: ${testBlog.likes}`)
  ...
  test('shows details when show details button is clicked', async () => {
    ...
    expect(container).toHaveTextContent(`url: ${testBlog.url}likes: ${testBlog.likes}`)
  })
